...
기존 Console 기반 미션을 웹 UI 환경으로 재해석하여, 재고 관리, 프로모션 계산, 영수증 출력 등의 핵심 비즈니스 로직을 모던 프론트엔드 기술 스택으로 구현한다. 콘솔 입출력은 브라우저 화면의 입력 폼과 UI 컴포넌트로 대체되며, 사용자 경험을 향상시킨 인터랙티브한 편의점 POS 시스템을 목표로 한다.
