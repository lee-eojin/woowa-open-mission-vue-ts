...
이 프로젝트는 **객체지향 설계 원칙**을 최대한 따르는 것을 목표로 한다. 상품(Product), 프로모션(Promotion), 재고(Inventory), 영수증(Receipt) 등 도메인 개념을 명확한 클래스로 분리하고, 각 클래스가 단일 책임을 가지도록 설계한다. TypeScript의 타입 시스템과 인터페이스를 활용해 객체 간 협력 관계를 명확히 정의하며, 캡슐화와 추상화를 통해 유지보수 가능한 코드를 작성한다.

## 기술 스택 및 선정 이유

### Vue 3 (Composition API)

- **컴포넌트 재사용성**: Composition API를 통해 로직을 함수 단위로 분리하고 재사용할 수 있어, 상품 카드, 장바구니, 영수증 등 반복되는 UI 로직을 효율적으로 관리
- **반응형 시스템**: 명시적인 반응형 데이터 관리로, 재고 변경이나 프로모션 적용 같은 실시간 업데이트가 필요한 로직 구현에 적합
- **Template 문법**: JSX가 아닌 HTML 기반 템플릿 문법으로 디자이너와의 협업이나 마크업 작업이 직관적
- **학습 곡선**: React보다 개념이 단순하고 공식 문서가 체계적이어서, 2주 내 프로젝트 완성에 유리하다고 판단

### TypeScript

- **새로운 도전**: JavaScript만 사용해본 상태에서 타입 시스템을 학습하고 적용해보는 첫 경험. 우테코 프리코스를 통해 처음 접한 코딩 경험을 타입 안전성이라는 새로운 개념으로 확장하는 기회
- **타입 안전성**: 상품, 프로모션, 재고 등 복잡한 도메인 객체의 구조를 인터페이스로 명확히 정의하여 런타임 에러 방지
- **IDE 지원**: 자동완성과 타입 추론으로 개발 속도 향상 및 오타로 인한 버그 감소
- **리팩토링 안정성**: 클래스나 인터페이스 변경 시 영향받는 모든 코드를 컴파일 타임에 파악 가능
- **객체지향 구현**: class, interface, abstract 등 OOP 개념을 언어 레벨에서 지원하여 도메인 모델 설계에 유리
