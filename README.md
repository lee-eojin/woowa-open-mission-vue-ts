...
- 상품 담기
  - 각 상품별 수량 입력 후 "담기" 버튼으로 추가
  - 여러 상품을 순차적으로 담아 다중 상품 구매 가능
  - 같은 상품 재담기 시 수량 업데이트
- 장바구니 관리
  - 담긴 상품 목록 실시간 표시
  - 개별 상품 삭제 기능
...
- 추가 구매 지원
  - 영수증 확인 후 자동으로 장바구니 초기화
  - 즉시 새로운 상품 구매 가능
- 입력 제약 및 검증
  - 수량 입력 필드: 1개 이상, 재고 범위 내로 제한
  - 재고 부족 시 에러 메시지 표시
  - 데이터 파싱 검증 (CSV 형식, 필수 값, 데이터 타입)
  - 존재하지 않는 상품 구매 방지
...
```bash
# 저장소 클론
git clone https://github.com/{your-username}/woowa-open-mission-vue-ts.git
cd woowa-open-mission-vue-ts
